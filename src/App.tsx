...
import { Heart, Music, VolumeX, Sparkles, Edit3, Check, X } from 'lucide-react';
...
export default function App() {
  ...
  // Spectacular fireworks when clicking "Gửi cái ôm ấm áp cho anh"
  const handleSendHug = () => {
    setHugSent(true);
  ...
  return (
    <div className="min-h-screen bg-[#0e0c13] text-stone-100 flex flex-col font-sans selection:bg-rose-500/30 selection:text-rose-200 relative overflow-x-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute top-10 left-1/2 -translate-x-1/2 w-[550px] h-[550px] bg-rose-600/10 rounded-full blur-[150px]" />
        <div className="absolute bottom-20 right-10 w-[350px] h-[350px] bg-amber-500/5 rounded-full blur-[130px]" />
      </div>

      {/* Top Floating Music Controls */}
      <aside aria-label="Điều khiển âm thanh" className="fixed top-4 right-4 z-40 flex items-center gap-2">
        <button
          ...
          className={`px-3 py-1.5 rounded-full transition-all text-xs flex items-center gap-1.5 shadow-lg backdrop-blur-md ${
            isPlayingMusic
              ? 'bg-rose-600/90 text-white shadow-rose-950/60'
              : 'bg-stone-900/80 text-stone-400 hover:text-stone-200 border border-stone-800'
          }`}
          title={isPlayingMusic ? 'Tạm dừng bài hát' : 'Phát Ngày Đầu Tiên'}
        >
          <Music className={`w-3.5 h-3.5 ${isPlayingMusic ? 'animate-bounce text-rose-200' : ''}`} />
          <span className="text-[11px] font-medium pr-0.5">
            {isPlayingMusic ? 'Ngày Đầu Tiên' : 'Bật nhạc'}
          </span>
        </button>

        <button
          onClick={handleToggleMute}
          className="p-2 rounded-full bg-stone-900/80 text-stone-400 hover:text-stone-200 border border-stone-800 transition-colors shadow-lg backdrop-blur-md"
          title={isMuted ? 'Bật tiếng' : 'Tắt tiếng'}
        >
          {isMuted ? <VolumeX className="w-3.5 h-3.5 text-stone-500" /> : <Sparkles className="w-3.5 h-3.5 text-rose-400" />}
        </button>
      </aside>

      {/* MAIN CONTENT: SIMPLE, DEEPLY ROMANTIC & TOUCHING */}
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 sm:px-6 py-12 sm:py-16 space-y-12 sm:space-y-14 relative z-20">
        ...
        <header className="text-center space-y-3 pt-2">
          ...
          {/* Recipient's Name (Editable with 1 click) */}
          <div className="flex items-center justify-center gap-2 pt-1">
            {isEditingName ? (
              <div className="flex items-center gap-1.5 bg-stone-900 border border-rose-500/40 rounded-full px-3 py-1">
                <input
                  type="text"
                  value={tempName}
                  onChange={(e) => setTempName(e.target.value)}
                  placeholder="Tên gọi thân mật..."
                  className="bg-transparent text-sm text-rose-200 focus:outline-none w-36 text-center font-serif"
                  autoFocus
                />
                <button onClick={handleSaveName} className="text-emerald-400 hover:text-emerald-300 p-0.5">
                  <Check className="w-4 h-4" />
                </button>
                <button onClick={() => setIsEditingName(false)} className="text-stone-400 hover:text-stone-300 p-0.5">
                  <X className="w-4 h-4" />
                </button>
              </div>
            ) : (
              ...
              >
                ...
                <Edit3 className="w-3.5 h-3.5 text-stone-500 opacity-60 group-hover:opacity-100 group-hover:text-rose-400 transition-all" />
              </div>
            )}
          </div>
        </header>
        ...
        {/* SECTION 2: 7-YEAR ANNIVERSARY PHOTO ALBUM */}
        <SevenYearGallery />

        {/* SECTION 3: DEDICATED ROMANTIC MUSIC PLAYER - "NGÀY ĐẦU TIÊN" (ĐỨC PHÚC) */}
        <NgayDauTienPlayer />

        {/* SECTION 4: THE INTIMATE HANDWRITTEN LOVE LETTER */}
        <section aria-label="Bức thư tình" className="space-y-3">
          <div className="flex items-center justify-between px-1">
            ...
            <button
              onClick={() => {
                setTempLetter(letterContent);
                setIsEditingLetter(!isEditingLetter);
              }}
              className="text-xs text-stone-400 hover:text-stone-200 transition-colors flex items-center gap-1 font-sans"
            >
              <Edit3 className="w-3 h-3" />
              <span>{isEditingLetter ? 'Hủy' : 'Sửa thư'}</span>
            </button>
          </div>
          ...
          <div className="bg-[#faf6ee] text-[#2c2420] rounded-2xl p-7 sm:p-9 shadow-2xl border border-[#d6c7b2] relative select-text">
            ...
            {isEditingLetter ? (
              <div className="space-y-3 font-sans text-xs">
                <label className="block text-stone-700 font-semibold">Lời nhắn gửi tới em:</label>
                <textarea
                  rows={10}
                  value={tempLetter}
                  onChange={(e) => setTempLetter(e.target.value)}
                  className="w-full p-3 bg-white border border-[#c5b59e] rounded text-stone-800 text-sm leading-relaxed focus:outline-none focus:border-rose-600"
                />
                <div className="flex justify-end gap-2 pt-1">
                  <button
                    onClick={() => setIsEditingLetter(false)}
                    className="px-3 py-1.5 bg-[#e0d6c5] text-stone-800 rounded text-xs"
                  >
                    Hủy
                  </button>
                  <button
                    onClick={handleSaveLetter}
                    className="px-4 py-1.5 bg-[#b34045] text-white rounded text-xs font-semibold flex items-center gap-1"
                  >
              ...
              <div className="font-serif space-y-4">
                <div className="text-xl sm:text-2xl font-bold text-[#1f1610]">
                  Gửi {herName},
                </div>

                <div className="text-base sm:text-lg leading-relaxed text-[#3a2d24] whitespace-pre-line text-justify">
                  {letterContent}
                </div>

                <div className="pt-6 border-t border-[#dccfbd] flex flex-col items-end text-right">
                  <div className="text-sm italic text-[#705445]">Người yêu em nhất trần đời,</div>
                  <div className="font-handwriting text-2xl sm:text-3xl text-[#9c2938] font-bold mt-1">
                    Anh của em
                  </div>
                  <div className="text-xs text-[#8a725b] mt-0.5 font-mono">25.09.2019 — 25.09.2026</div>
                </div>
              </div>
            )}
          </div>
        ...
        {/* SECTION 5: THE SPECIAL INTERACTION - "GỬI CÁI ÔM ẤM ÁP CHO ANH" + FIREWORKS */}
        <section
          aria-label="Tương tác gửi cái ôm"
          className={`p-7 sm:p-10 rounded-2xl bg-gradient-to-b from-stone-900/90 to-stone-950/90 border transition-all duration-500 text-center space-y-5 shadow-2xl relative overflow-hidden ${
            isFireworksActive
              ? 'border-rose-400 shadow-2xl shadow-rose-600/30 scale-[1.01]'
              : 'border-rose-500/30'
          }`}
        >
          {isFireworksActive && (
        ...
        {/* FOOTER */}
        <footer className="text-center pt-6 pb-8 border-t border-stone-800/60 text-xs text-stone-500 font-sans space-y-1">
